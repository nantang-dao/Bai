<template>
  <div class="min-h-screen pb-24 px-4 py-6 max-w-lg mx-auto">
    <header class="flex items-center gap-3 mb-6">
      ...
      <h1 class="text-xl font-bold text-text-title">商城标签管理</h1>
    </header>
...
const editing = ref<MarketplaceTag | null>(null)
const editName = ref('')
const editColor = ref('#000000')
const saving = ref(false)

function openEdit(t: MarketplaceTag) {
  editing.value = t
...
async function createTag() {
  ...
  creating.value = true
  try {
    await api.createMarketplaceTag(communityId.value, { name: newName.value.trim(), colorHex: newColor.value })
    newName.value = ''
...
async function saveEdit() {
  ...
  saving.value = true
  try {
    await api.updateMarketplaceTag(communityId.value, editing.value.id, {
...
async function removeTag() {
  if (!editing.value) return
  try {
    await api.deleteMarketplaceTag(communityId.value, editing.value.id)
...
onMounted(async () => {
  ...
  const role = communityStore.currentCommunity?.myRole
  if (role !== 'super_admin') {
