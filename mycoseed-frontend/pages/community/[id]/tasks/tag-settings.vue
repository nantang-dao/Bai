<template>
  <div class="min-h-screen pb-24 px-4 py-6 max-w-lg mx-auto">
    <header class="flex items-center gap-3 mb-6">
      ...
      <h1 class="text-xl font-bold text-text-title">任务标签管理</h1>
    </header>
...
import type { TaskTag } from '~/utils/api'
...
const editing = ref<TaskTag | null>(null)
const editName = ref('')
const editColor = ref('#000000')
const saving = ref(false)

function openEdit(t: TaskTag) {
  editing.value = t
...
async function load() {
  tags.value = await api.listTaskTags(communityId.value)
}
...
async function createTag() {
  ...
  creating.value = true
  try {
    await api.createTaskTag(communityId.value, { name: newName.value.trim(), colorHex: newColor.value })
    newName.value = ''
...
async function saveEdit() {
  ...
  saving.value = true
  try {
    await api.updateTaskTag(communityId.value, editing.value.id, {
...
async function removeTag() {
  if (!editing.value) return
  try {
    await api.deleteTaskTag(communityId.value, editing.value.id)
...
onMounted(async () => {
  ...
  if (communityStore.currentCommunity?.myRole !== 'super_admin') {
    router.replace(`/community/${communityId.value}/tasks`)
    return
  }
  await load()
})
