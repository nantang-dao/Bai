...
/// @notice TaskPool V4（通过 Proxy 使用）
/// V4 变更要点（兼容 V3 存储布局）：
/// - Manager 日常子任务审核链下完成；链上仅保留必要的取消/过期等闭环操作。
/// - Publisher 在「终局通过」时一笔交易完成：批量将仍 Open 且已认领的子任务置为 Completed + 开启公示期。
/// - 公示窗口长度由管理员可配置（`publicizeWindowSeconds`），测试默认 60 秒。
contract TaskPoolLogicV4 {
    // ========= EIP-712 (Typed Data) =========
    ...
    /// @notice 公示窗口时长（秒）。可由管理员更新；测试默认 60 秒。
    /// @dev 追加在存储尾部，避免破坏 Proxy 存储布局。
    uint64 public publicizeWindowSeconds;
    ...
    event PublicizeWindowSecondsSet(uint64 seconds_);
    ...
    error UnclaimedOpenTask(uint256 taskId);
    ...
    function setPublicizeWindowSeconds(uint64 seconds_) external onlyAdmin {
        require(seconds_ > 0, "ZERO_WINDOW");
        publicizeWindowSeconds = seconds_;
        emit PublicizeWindowSecondsSet(seconds_);
    }
    ...
    // ========= Subtask review (legacy onchain path; keep for admin/compat) =========
    ...
    function finalApprovePool(
        ...
        _revertIfAnyUnclaimedOpen(poolId);

        // 1) 批量将仍 Open 且已认领的子任务置为 Completed（整包通过）
        uint256 len = p.taskIds.length;
        for (uint256 i = 0; i < len; i++) {
            uint256 tid = p.taskIds[i];
            Task storage t = poolTasks[poolId][tid];
            if (!t.exists) continue;
            if (t.status != TaskStatus.Open) continue;
            if (t.assignee == address(0)) continue; // 已在 _revertIfAnyUnclaimedOpen 里保证不存在未认领 Open
        ...
        uint64 end = start + (publicizeWindowSeconds == 0 ? 60 : publicizeWindowSeconds);
    ...
    function _revertIfAnyUnclaimedOpen(uint256 poolId) internal view {
        Pool storage p = pools[poolId];
        uint256 len = p.taskIds.length;
        for (uint256 i = 0; i < len; i++) {
            uint256 tid = p.taskIds[i];
