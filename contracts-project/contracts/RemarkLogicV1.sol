...
///
/// ### 这个合约解决什么问题？
/// - 你的业务里，接包者在「提交凭证」时会写一句话；发包者对整单可写一句总评。
/// - 链下收集后，由 Publisher 终审开启公示的那笔交易批量写入区块链。
/// - 与 TaskPool 约定：taskId==0 表示整池总评（写入 receiverRemarks）；taskId>0 为子任务（接包者话在 senderRemarks）。
/// - 本合约只负责把文字与 (poolId, taskId) 绑定存链上，不负责资金结算（由 TaskPool 负责）。
///
